...
export function AuthProvider({ children }) {
    ...
    const login = useCallback(async (email, password) => {
        ...
        const { password: _, password_hash: __, ...safeUser } = found;
        setUser(safeUser);
    ...
    const register = useCallback(async (data) => {
        ...
        const { password: _, password_hash: __, ...safeUser } = newUser;
        setUser(safeUser);
        localStorage.setItem('hitcell_user', JSON.stringify(safeUser));
    ...
    const isAdmin = user?.type === 'employee' || user?.type === 'admin' || user?.role_id?.startsWith('role-');
    const isCustomer = user?.type === 'customer' || user?.role_id === 'customer';

    const hasPermission = useCallback((perm) => {
        if (!user) return false;
        if (user.role_id === 'role-admin' || user.role === 'admin' || user.type === 'admin') return true;
